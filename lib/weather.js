...
var NewTabWeather = {
    ...
    initWeather: function(worker) {
        ...
        var conditionsIcon = ss.storage[WEATHER_CONDITIONS_ICON_SS];
        ...
        if(conditionsIcon && locationName && temperature && cachedTemperatureUnits) {
            logger.log('Sending cached weather result.');
            NewTabWeather.sendWeatherResult(worker, conditionsIcon, locationName, temperature, 
        ...
        if(NewTabWeather.shouldUpdate()) {
            ...
            NewTabWeather.getLocation(worker, location, function(position){
                ...
                NewTabWeather.getWeather(worker, position, 
     ...
     */
    getWeather: function(worker, position, temperatureUnits, cb) {
        //build request URL
        ...
        Request({
            ...
            onComplete: function(response) {
                ...
                var conditionsIcon = NewTabWeather.getConditionsIcon(json.weather);
                NewTabWeather.cacheWeatherResult(conditionsIcon, json.name,
                    temperature, temperatureUnits);
                cb(worker, conditionsIcon, json.name, temperature, temperatureUnits);
            }
        }).get();
    },
     ...
     */
    cacheWeatherResult: function(conditionsIcon, locationName, temperature, temperatureUnits) {
        if(!conditionsIcon || !locationName || !temperature || !temperatureUnits) {
        ...
        ss.storage[WEATHER_CONDITIONS_ICON_SS] = conditionsIcon;
      ...
      */
    sendWeatherResult: function(worker, conditionsIcon, locationName, temperature, temperatureUnits) {
        if(!conditionsIcon || !locationName || !temperature || !temperatureUnits) {
