...
var Weather = {
    ...
    initWeather: function(worker) {
        ...
        if(weatherVisibility === 'never') {
            worker.port.emit(HIDE_WEATHER_MSG);
            return;
        }
        ...
        if(conditions && locationName && temperature && cachedTemperatureUnits) {
            ...
            Weather.sendWeatherResult(worker, conditions, locationName, temperature, 
                cachedTemperatureUnits);
        }

        //request weather update if cached result is stale
        if(Weather.isCachedWeatherResultStale()) {

            //indicate that the weather is being updated
            Weather.showLoadingSpinner(worker);

            //get user's location
            var location = simplePrefs.prefs[LOCATION_PREF];
            Weather.getLocation(worker, location, function(position){

                //get updated weather and send result to content scripts
                var temperatureUnitsPref = simplePrefs.prefs[TEMPERATURE_UNITS_PREF];
                Weather.getWeather(worker, OPENWEATHERMAP_APPID, position, 
                    temperatureUnitsPref, Weather.sendWeatherResult);
            });
        }
    },
    ...
    showLoadingSpinner: function(worker) {
        worker.port.emit(WEATHER_SHOW_LOADING_MSG);
    },
    ...
    getLocation: function(worker, location, cb) {
        ...
        worker.port.on(WEATHER_GEOLOCATION_RESULT_MSG, function(coords) {
            ...
            if(!coords || !coords.latitude || !coords.longitude) {
                ...
                Weather.sendEmptyWeatherResult(worker);
                return;
            }
    ...
    getWeather: function(worker, appId, position, temperatureUnits, cb) {
        ...
        Request({
            ...
            onComplete: function(response) {
                ...
                Weather.cacheWeatherResult(json.weather, json.name, temperature, temperatureUnits);
    ...
    sendWeatherResult: function(worker, conditions, locationName, temperature, temperatureUnits) {
        ...
        var options = {
            ...
            temperatureUnits: Weather.getTemperatureUnitsStr(temperatureUnits)
        };
        options[SHOW_WEATHER_PREF] = simplePrefs.prefs[SHOW_WEATHER_PREF];
        worker.port.emit(WEATHER_MSG, options);
    },
...
simplePrefs.on(LOCATION_PREF, Weather.clearCachedWeatherResult);
simplePrefs.on(TEMPERATURE_UNITS_PREF, Weather.clearCachedWeatherResult);
