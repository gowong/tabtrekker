 ...
 * is being updated.
 */
 function showLoadingSpinner(worker) {
...
function getLocation(worker, location, cb) {
    ...
    worker.port.on(WEATHER_GEOLOCATION_RESULT_MSG, function(coords) {
        ...
        if(!coords || !coords.latitude || !coords.longitude) {
            sendEmptyWeatherResult(worker, null);
            return;
        }
...
function getWeather(worker, appId, position, temperatureUnits, cb) {
    ...
    Request({
        ...
        onComplete: function(response) {
            ...
            cacheWeatherResult(json.weather, json.name, temperature, temperatureUnits);
 ...
 */
 function sendEmptyWeatherResult(worker, locationName) {
    locationName = locationName || simplePrefs.prefs[LOCATION_PREF] || '';
    worker.port.emit(WEATHER_MSG, {
        conditions: null,
        location: locationName,
        temperature: '--',
